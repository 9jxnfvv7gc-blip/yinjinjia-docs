...
- ✅ 调试信息已添加
- ✅ 语法正确
- ❌ `VIDEO_ROOT` 配置错误：显示 `/Volumes/Expansion`，应该是 `/root/videos`
...
1. **停止服务器**（如果手动运行中，按 `Ctrl+C`）
2. **修复VIDEO_ROOT配置**（`sed -i 's|VIDEO_ROOT = "/Volumes/Expansion"|VIDEO_ROOT = "/root/videos"|g' video_server.py`）
3. **验证修改**（`grep "VIDEO_ROOT" video_server.py`）
