...
class _SimpleHomePageSafeState extends State<SimpleHomePageSafe> {
      ...
      if (mounted) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Text('无法连接到服务器，当前暂无内容。您可以继续使用其他功能。'),
  ...
  Future<http.Response> _getWithRetry(String url, {int maxRetries = 3, Duration? delay, String? type}) async {
    ...
    for (int i = 0; i < maxRetries; i++) {
      try {
        if (kDebugMode) {
          print('🔍 准备发送HTTP请求: $url');
          print('🔍 请求头: Accept: application/json, User-Agent: VideoMusicApp/1.0');
    ...
    try {
      ...
      try {
        if (kIsWeb) {
          ...
          // iOS平台：使用any类型，让用户可以从文件应用选择
          // iOS 上 FileType.audio 可能导致崩溃，改用 FileType.any
          if (kDebugMode) {
            print('iOS 文件选择器：类型=$type');
          }
          // iOS 上使用 FileType.any，让用户从文件应用选择
          // 注意：iOS 上不能使用 allowedExtensions
          result = await FilePicker.platform.pickFiles(
            type: FileType.any,
            allowMultiple: true,
            // iOS 上不能使用 allowedExtensions
          );
        } else {
            ...
            return SingleChildScrollView(
              ...
              child: ConstrainedBox(
                constraints: BoxConstraints(
                  minHeight: constraints.maxHeight - 32, // 减去 padding
                ),
                child: Column(
