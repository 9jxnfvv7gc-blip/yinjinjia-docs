    ...
    for (int i = 0; i < maxRetries; i++) {
      try {
        ...
        try {
          ...
          print('🔍 完整URL: $url');
          print('🔍 协议: ${testUri.scheme}');
          print('🔍 主机: ${testUri.host}');
          print('🔍 端口: ${testUri.port}');
          
          // iOS上测试DNS解析
          if (Platform.isIOS) {
            try {
              final addresses = await InternetAddress.lookup(testUri.host);
              print('✅ DNS解析成功: ${testUri.host} -> ${addresses.map((a) => a.address).join(", ")}');
            } catch (dnsError) {
              print('❌ DNS解析失败: $dnsError');
              print('   这可能是网络问题的根本原因');
            }
          }
        } catch (e) {
        ...
        print('🔍 开始发送请求...');
        print('🔍 客户端类型: ${client.runtimeType}');
